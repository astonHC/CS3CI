...
static double TSP_ACO_PROB(TSP_STATE* STATE, TSP_ACO_STATE* ACO_STATE, int CURRENT, int NEXT, int* VISITED)
{
    ...
    double DIST_PROB = STATE->DIST.MATRIX[CURRENT][NEXT];

    if(DIST_PROB == 0)
    {
        TSP_HANDLE(ACO, TSP_ERROR_ACO, " -> INVALID DISTANCE FROM CITY: %d TO CITY: %d", CURRENT, NEXT);
    }
...
{
    double UNVISITED[TSP_MAX_CITIES];
    int COUNT = 0;
    int RESULT = -1;
...
int TSP_ACO_BASE(TSP_STATE* STATE)
{
    ...
    int MAX_ITER = TSP_ACO_ITER_RATE;
    int ITERATION = 0;
    int IMPROVEMENTS = 0;
    ...
    for(ITERATION = 0; ITERATION < MAX_ITER; ITERATION++)
    {
        ...
        // TRACK THE ITERATIONS AND APPLY THEM TO THE COUNT 
        int PREV_BEST = ACO_STATE->BEST_DIST;
        ...
        if(ACO_STATE->BEST_DIST < PREV_BEST) IMPROVEMENTS++;
    }

    TSP_HANDLE(NONE, TSP_ERROR_NONE, 
                "ACO HAS REACHED OPTIMAL PATH AFTER %d ITERATIONS, IMPROVEMENTS: %d\n", 
