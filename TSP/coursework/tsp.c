...
int TSP_NEAREST(TSP_STATE* STATE) 
{
    ...
    for(int STEP = 1; STEP < STATE->CITY_COUNT; STEP++)
    {
        ...
        double NEAR_DIST = INT_MAX;    // WE WILL NEVER REACH INF
        ...
        for(int INDEX = 0; INDEX < STATE->CITY_COUNT; INDEX++)
        {
            ...
            double EDGE_DIST = STATE->DIST.MATRIX[CURRENT_CITY][INDEX];
        ...
        TSP_HANDLE(NONE, TSP_ERROR_NONE, "NEAREST CITY: %s WITH DISTANCE: %.2f", STATE->CITY[NEAREST].NAME, NEAR_DIST);
    ...
    double RETURN_DIST = STATE->DIST.MATRIX[CURRENT_CITY][0];
    BASE_DIST += RETURN_DIST;

    TSP_HANDLE(NONE, TSP_ERROR_NONE, "RETURN TRIP: %s TO %s WITH DISTANCE: %.2f\n", 
    ...
    TSP_HANDLE(DIST, TSP_ERROR_NONE, "DISTANCE COVERED %.2f\n", STATE->TOUR.TSP_TOTAL_DIST);
