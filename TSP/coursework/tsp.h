...
#include <time.h>
    ...
    // STRING COMPARISON MACROS FOR HANDLING 
    // ALGORITHM TYPES AND THEIR RESPECTIVE CHARACTERISTICS
    // AT RUNTIME
    // THIS ALSO HELPS WITH BEING ABLE TO FIND ALGORITHMS BASED ON THEIR 
    // RESPECTIVE ASSOCIATIONS USING STATIC ACCESS 
    #define     TSP_FIND_ALGO(DATA, NAME)   \
                ((strcmp((DATA)->ALGORITHM_A.NAME, (NAME)) == 0) ? (DATA)->ALGORITHM_A : \
                (strcmp((DATA)->ALGORITHM_B.NAME, (NAME)) == 0) ? (DATA)->ALGORITHM_B : NULL)
