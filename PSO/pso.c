...
// BOUNDS ARE TRADITIONALLY SET FOR THE GIVEN PHASE WEIGHTS
void PSO_SET_BOUNDS(PSO* PSO_STATE, int DIMENSION, double LOWER, double UPPER)
{
    ...
    PSO_HANDLE(NONE, PSO_ERROR_NONE, "PSO SET WITH BOUNDS FOR ANTENNA: %d: [L] %.2f -> [R] %.2f", DIMENSION, LOWER, UPPER);
}
...
static double PSO_ADAPTIVE_WEIGHT(PSO* PSO_STATE)
{
    ...
    if(WEIGHT > PSO_INERTIA_MAX) WEIGHT = PSO_INERTIA_MAX;
    if(WEIGHT < PSO_INERTIA_MIN) WEIGHT = PSO_INERTIA_MIN;
...
static void PSO_UPDATE_PARTICLE(PSO* PSO_STATE, PSO_PARTICLE* PARTICLE, double INERTIA_WEIGHT)
{
    for(int DIMENSIONS = 0; DIMENSIONS < PSO_STATE->DIMENSIONS; DIMENSIONS++)
    {
        ...
        PARTICLE->VELOCITY[DIMENSIONS] = (INERTIA_WEIGHT * PARTICLE->VELOCITY[DIMENSIONS]) + COGNITIVE + SOCIAL;
