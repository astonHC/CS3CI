...
{
    const double WAVELENGTH = PSO_ANTENNA_WLENGTH;
    ...
    for(int SAMPLE = 0; SAMPLE < PSO_ANTENNA_SAMPLES; SAMPLE++)
    {
        ...
        // SKIP MAIN LOBE IF WE FOUND AN ABSOLUTE VALUE PERTAINING
        // TOWARDS THE ANGLE
        ...
        for(int INDEX = 0; INDEX < NUM_ANTENNAE; INDEX++)
        {
            ...
            IMAG_SUM += sin(TOTAL_PHASE);
        }
        
        // DEFINE THE SIDE LOBE AND ASSIGN IT TO THE MAX
        // THIS WILL BE OUR CONVERGENCE
        double SLL = sqrt(REAL_SUM * REAL_SUM + IMAG_SUM * IMAG_SUM) / (double)NUM_ANTENNAE;
