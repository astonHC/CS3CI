...
static void PSO_UPDATE_INERTIA(PARTICLE P, const double* GBEST, int DIMENSIONS, double INERTIA, double COG, double SOC)
{
    ...
    for(int INDEX = 0; INDEX < DIMENSIONS; INDEX++)
    {
        ...
        // ASSIGN THE VELOCITY TO THE COMPONENTS IN ACCORDANCE
        // WITH THE INERTIA 
        P->VELOCITY[INDEX] = (INERTIA * P->VELOCITY[INDEX]) + COG_COMP + SOC_COMP;
    }
}

// NOW DO THE SAME BY FOR THE POSITION OF EACH PARTICLE
// WITHIN THE SWARM 
...
static void PSO_UPDATE_ADAPT(STATE S)
{
    ...
    S->CURRENT_INERTIA = PSO_MAX_INERTIA - (PSO_MAX_INERTIA - PSO_MIN_INERTIA) * PROGRESS;

    // MAP ALL OF THE COEFFICIENTS TO THE NOVEL COMPONENTS
    S->CURRENT_COG = PSO_COG_INIT - (PSO_COG_INIT - PSO_COG_FIN) * PROGRESS;
    S->CURRENT_SOC = PSO_SOC_INIT - (PSO_SOC_FIN - PSO_SOC_INIT) * PROGRESS;
}
...
static void PSO_UPDATE_GBEST(STATE S)
{
    ...
    for(INDEX = 0; INDEX < PSO_MAX_PARTICLES; INDEX++)
    {
        ...
        if(P->PBEST_FITNESS < S->SWARM.GBEST_FITNESS)
        {
            ...
            PSO_HANDLE(NONE, PSO_ERROR_NONE, 
                "NEW GLOBAL BEST FITNESS: %.2f ORIGINAL FITNESS: %2.f", 
                P->PBEST_FITNESS, P->CURRENT_FITNESS);
    ...
    // UPDATE STAGNATE COUNTER BASED ON THE AVERAGE ABSOLUTE ERROR BIAS
    if(fabs(S->SWARM.GBEST_FITNESS - PREV_GBEST) < DBL_MAX)
    {
        S->SWARM.STAGNATE++;
    }
    else
...
void PSO_OPTIMISE(PSO_STATE* STATE)
{
    ...
    PSO_HANDLE(NONE, PSO_ERROR_NONE, "INITIAL BEST FITNESS: %.6f\n", STATE->SWARM.GBEST_FITNESS);
    ...
    for(int INDEX = 0; INDEX < PSO_MAX_ITER; INDEX++)
    {
        ...
        // UPDATE ALL OF THE PARTICLES TO LEVERAGE
        // THE NOVEL VARIANTS
...
int PSO_LOAD_CSV(PSO_DATASET* DATASET, const char* FILENAME)
{
    ...
    {
        fclose(FILE_PTR);
        PSO_ERROR_HANDLE(MEM, PSO_ERROR_MEM, "FAILED TO ALLOCATE MEMORY FOR DATASET", " ");
        return 1;
    }
    ...
    fclose(FILE_PTR);
    PSO_HANDLE(NONE, PSO_ERROR_NONE, 
        "LOADED %d RECORDS FROM %s", DATASET->SIZE, FILENAME);
