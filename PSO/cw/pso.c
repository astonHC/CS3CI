...
static double PSO_SWARM_DIVERSITY(STATE S)
{
    ...
    for(int SEARCH = 0; SEARCH < S->DIMENSIONS; SEARCH++)
    {
        ...
        SEARCH_RANGE = S->BOUNDS[SEARCH].UPPER - S->BOUNDS[SEARCH].LOWER;
...
static void PSO_UPDATE_ADAPT(STATE S)
{
    ...
    if(S->SWARM.DIVERSITY < PSO_DIV_THRES)
    {
        ...
        PSO_HANDLE(NONE, PSO_ERROR_NONE,
            "ITERATION: %d -> LOW DIVERSITY -> FROM: %.4f TO: %.4f - BOOSTED TO: %.4f\n",
                S->SWARM.ITERATION, S->SWARM.DIVERSITY, PSO_DIV_THRES, S->CURRENT_INERTIA);
    }
