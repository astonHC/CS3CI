...
int PSO_INIT(PSO_STATE* STATE, int DIMENSIONS)
{
    ...
    STATE->SWARM.PARTICLES->CURRENT_FITNESS = DBL_MAX;
    STATE->SWARM.PARTICLES->PBEST_FITNESS = DBL_MAX;
    ...
    STATE->CURRENT_COG = PSO_COG_MAX;
    STATE->CURRENT_SOC = PSO_SOC_MAX;
...
{
    if(!PSO_VALID_DIM(DIMENSION, PSO_MAX_DEM))
    {
...
static void PSO_INIT_PARTICLE(PSO_PARTICLE* PARTICLE, const PSO_BOUND* BOUNDS, int DIMENSIONS)
{
    ...
    for(int INDEX = 0; INDEX < DIMENSIONS; INDEX++)
    {
        ...
        PARTICLE->VELOCITY[INDEX] = (PSO_RAND() * (2.0 * PSO_VELO_DELTA)) - VELOCITY_MAX;
...
double PSO_PREDICT(const double* PARAMS, const double* INDICATIONS)
{
    ...
    // MANUAL CLAMP AGAINST PREDICTION BIAS
...
double PSO_DEMAND_FITNESS(const double* PARAMS, const PSO_DATASET* DATASET)
{
    ...
    for(int INDEX = 0; INDEX < DATASET->SIZE; INDEX++)
    {
        ...
        // DETERMINE THE PREDICITION BIAS TO THE AMOUNT OF TRUCKS
        // THIS WORKS IN TANDEM WITH THE CURRENT DATASET
        double PREDICTED_TRUCKS = ceil(PREDICTION / PSO_TRUCK);
        double ACTUAL_TRUCKS = ceil(DATASET->DATA[INDEX].DEMANDS / PSO_TRUCK);

        // CALCULATE THE TOTAL DEMAND PREDICTION FROM THE DATASET
        TRUCK_ERROR = fabs(PREDICTED_TRUCKS - ACTUAL_TRUCKS);
        
        // ASSIGN THE ABOVE TO THE TOTAL ERROR COST
        TOTAL_COST += TRUCK_ERROR;
    } 

    // ASSIGN THE AVERAGE ABSOLUTE AGAINST THE SIZE OF THE DATASET
    // THIS WILL DETERMINE THE EFFECTIVENESS OF THE SAVED COST
    MAE = TOTAL_COST / DATASET->SIZE;

    #if PSO_DEBUG
    PSO_HANDLE(NONE, PSO_ERROR_NONE, "AVERAGE TRUCK COST FOR PREDICTION: %.2f = %.2f\n", PREDICTION, MAE);
    #endif
...
static double PSO_SWARM_DIVERSITY(STATE S)
{
    ...
    // NOW WE CAN DETERMINE THE NORMALISED DISTANCE 
    // IN THE SEARCH SPACE
    for(int SEARCH = 0; SEARCH < S->DIMENSIONS; SEARCH++)
    {
        // DEFINE THE RNAGE FOR HOW WE CAN VALIDATE THIS DIVERSITY
        SEARCH_RANGE = PSO_CLAMP_BOUNDS(S->BOUNDS, SEARCH);
        SEARCH_SPACE += SEARCH_RANGE * SEARCH_RANGE;
    }

    SEARCH_SPACE = sqrt(SEARCH_SPACE);

    // RETURN THIS NORMALISED DIVERSITY SCALE BASED ON THE 
    // EUCLIDEAN DISTANCE BETWEEN BOUNDS
    double NORM_DIST = (COMP > 0) ? (AVG_DIST / COMP) : 0.0;
    double NORM_DIV = (SEARCH_SPACE > 0) ? (NORM_DIST / SEARCH_SPACE) : 0.0;

    return NORM_DIV;
}

// MUTUATE A STAGNANT PARTICLE TO HELP WITH THE ESCAPE FROM LOCAL OPTIMA
// PRESUPPOSE THAT THROUGH EACH ITERATION, THERE WILL ALWAYS BE
// A CLUSTER CAUSING INADVERTENT CONVERGENCE
// 
// WE WANT TO BE ABLE TO HELP MITIGATE THIS BY
// RANDOMLY REINITIALISING A PORTION OF THE PARTICLE'S DIMENSIONS
// TO HELP WITH EXPLORATION WHEN STUCK
static void PSO_MUTATE_PARTICLE(PARTICLE P, const PSO_BOUND* BOUNDS, int DIMENSIONS)
{
    int MUTATIONS = 0;

    for(int INDEX = 0; INDEX < DIMENSIONS; INDEX++)
    {
        // MUTATE THE CURRENTLY SELECTED DIMENSION BASED ON MUT RATE
        if(PSO_RAND() < PSO_MUT_RATE)
        {
            double MUTATE_RANGE = PSO_CLAMP_BOUNDS(BOUNDS, INDEX);

            // REINITIALISE THE CURRENT DIMENSION WITHIN THE BOUNDS
            P->POSITION[INDEX] = BOUNDS[INDEX].LOWER + (PSO_RAND() * MUTATE_RANGE);
...
static void PSO_UPDATE_PBEST(PARTICLE P, int DIMENSIONS)
{
    ...
    if(P->CURRENT_FITNESS < P->PBEST_FITNESS)
    {
        ...
        #if PSO_DEBUG
        PSO_HANDLE(NONE, PSO_ERROR_NONE, 
            "NEW PERSONAL BEST FITNESS: %.2f ORIGINAL FITNESS: %.2f", 
            P->PBEST_FITNESS, P->CURRENT_FITNESS);
        #endif
...
static void PSO_UPDATE_ADAPT(STATE S)
{
    ...
    // LEVERAGE A NON-LINEAR MEANS OF DETERMINING THE ADAPTATION OVERTIME
    ...
    // OF THE ADAPTATION
    //
    // CREATE THE INITIAL SIGMOID CURVATURE TO HANDLE
    // SMOOTHER CONVERGENCE ITERATIONS OVERTIME
    double SIGMOID_EXPO = -PSO_ADAPT_PEAK * (PROGRESS - PSO_ADAPT_INTER);
    double SIGMOID = PSO_ADAPT_GRAD / (PSO_ADAPT_GRAD + exp(SIGMOID_EXPO));

    // LINEARLY DECREASE THE INERTIA WEIGHT TO GARNER
    // A GREATER CONTROL AGAINST THE VELOCITY OF THE PARTICLES
    S->CURRENT_INERTIA = PSO_MAX_INERTIA - (PSO_MAX_INERTIA - PSO_MIN_INERTIA) * SIGMOID;

    // TRACK THE CURRENT PROGRESS OF THE ADAPATION TO SEE
    // IF WE HAVE ANY VALID ADAPATIONS

    #if PSO_DEBUG
    PSO_HANDLE(NONE, PSO_ERROR_NONE, "ITER: %d PROGRESS: %.3f SIGMOID: %.4f INERTIA: %.4f",
    S->SWARM.ITERATION, PROGRESS, SIGMOID, S->CURRENT_INERTIA);
    #endif

    // FOCUS ON PROVIDING A BOSST FOR THE NATURAL INERTIA TO HELP
    // WITH A GREATER ADAPATION OF CONVERGENCE
    if(S->SWARM.DIVERSITY < PSO_DIV_THRES)
    {
        // CALCULATE SAID BOOST FACTOR BASED ON THE DIVERSITY LEVEL
        // LOWER DIVERSITY TRIGGERS STRONGER INTERVENTION
        double DIV_RATIO = S->SWARM.DIVERSITY / PSO_DIV_THRES;
        double BOOST = PSO_DIV_CLAMP + PSO_DIV_BOOST * (PSO_DIV_CLAMP - S->SWARM.DIVERSITY / DIV_RATIO);
        
        // APPLY SAID BOOST TO THE INERTIA - PREVENTS EXCESSIVE EXPLORATION
        S->CURRENT_INERTIA *= BOOST;

        // APPLY CAP TO MAINTAIN STABILITY BASED ON LOCAL OPTIMUM
        double INERTIA_CAP = PSO_CAP * PSO_MAX_INERTIA;
        if(S->CURRENT_INERTIA > INERTIA_CAP) S->CURRENT_INERTIA = INERTIA_CAP;
    ...
    S->CURRENT_COG = PSO_COG_MAX - (PSO_COG_MAX - PSO_COG_MIN) * SIGMOID;
    S->CURRENT_SOC = PSO_SOC_MAX + (PSO_SOC_MIN - PSO_SOC_MAX) * SIGMOID;
}
...
static void PSO_UPDATE_GBEST(STATE S)
{
    ...
    for(INDEX = 0; INDEX < PSO_MAX_PARTICLES; INDEX++)
    {
        ...
        if(P->PBEST_FITNESS < S->SWARM.GBEST_FITNESS)
        {
            ...
            PSO_HANDLE(NONE, PSO_ERROR_NONE, 
                "NEW GLOBAL BEST FITNESS: %.2f AT ITERATION %d -> ORIGINAL FITNESS: %.2f", 
    ...
    // ASSUME WE FIND ANY AND ALL IMPROVEMENTS
    // IN THE CURRENT GLOBAL BEST BASED ON ITERATION
    if(IMPROVEMENT)
    {
        if(PREV_GBEST == DBL_MAX)
        {
            PSO_HANDLE(NONE, PSO_ERROR_NONE,
                "INITIAL GLOBAL BEST: %.6f",
            S->SWARM.GBEST_FITNESS);
        }
        else
        {   
            PSO_HANDLE(NONE, PSO_ERROR_NONE, 
...
void PSO_OPTIMISE(PSO_STATE* STATE)
{
    ...
    // SAVE THE INITIAL DIVERSITY FROM THE INITIAL SWARM
    STATE->SWARM.DIVERSITY = PSO_SWARM_DIVERSITY(STATE);
    ...
    for(int INDEX = 0; INDEX < PSO_MAX_ITER; INDEX++)
    {
        ...
        for(int ITERATOR = 0; ITERATOR < PSO_MAX_PARTICLES; ITERATOR++)
        {
            ...
            // MUTATE HIGHLY STAGNANT PARTICLES
            // TO HELP MITIGATE EXCESS
            if(P->STAGNATE >= PSO_STAG_THRES)
            {
                #if PSO_DEBUG
                PSO_HANDLE(NONE, PSO_ERROR_NONE, "PARTICLE %d STAGNATED: %d\n", ITERATOR, P->STAGNATE);
                #endif

                PSO_MUTATE_PARTICLE(P, STATE->BOUNDS, STATE->DIMENSIONS);

                // NOW WE CAN RE-EVALUATE FITNESS AFTER SAID MUTATION
                P->CURRENT_FITNESS = PSO_DEMAND_FITNESS(P->POSITION, &STATE->DATASET);
                PSO_UPDATE_PBEST(P, STATE->DIMENSIONS);
                continue;
            }

            // UPDATE ALL OF THE CORRESPONDENCE AS EXPECTED
        ...
        // ONLY UPDATE GLOBALLY ONCE IT HAS REACHED GLOBAL OPTIMUM
        // ORIGINALLY, IT WAS SET SO THAT IT WOULD ONLY DO SO
        // AFTER EVERY ITERATION (0 AND MAX_ITER)
        PSO_UPDATE_GBEST(STATE);
    }
