...
double PSO_BASE_FITNESS(const double* POSITION, int DIMENSIONS, void* USER_DATA)
{
    ...
    for(int INDEX = 0; INDEX < STATE->DATASET.SIZE; INDEX++)
    {
        ...
        for(int ITERATOR = 0; ITERATOR < PSO_MAX_IND; ITERATOR++)
        {
            ...
            {
                // ASSIGN THE ARBITRARY PREDICTION BIAS AGAINST INDICATORS
        ...
        double ACTUAL_TRUCKS = ceil(ENTRY->DEMANDS / PSO_TRUCK);
        ...
        TOTAL_ERROR += TRUCK_ERROR;
        COUNTER++;
    }
    
    // ASSIGN THE MEAN ABSOLUTE AGAINST THE AMOUNT OF TRUCKS
    double MAE = (COUNTER > 0) ? (TOTAL_ERROR / COUNTER) : DBL_MAX;
    
    #if PSO_DEBUG
    PSO_HANDLE(NONE, PSO_ERROR_NONE, "AVERAGE TRUCK ERROR: %.6f", MAE);
    #endif
...
static void PSO_SIMPLE_PARTICLE(PARTICLE P, const BOUNDS B, int DIMENSIONS)
{
    ...
    for(int INDEX = 0; INDEX < DIMENSIONS; INDEX++)
    {
        ...
        P->VELOCITY[INDEX] = (PSO_RAND() * VELOCITY_MAX) * VELOCITY_MAX;
        P->PBEST[INDEX] = P->POSITION[INDEX];
        P->VELOCITY[INDEX] = VELOCITY_MAX;
    } 
...
{   
    ...
    {
        PSO_SIMPLE_PARTICLE(&STATE->SWARM.PARTICLES[INDEX], STATE->BOUNDS, STATE->DIMENSIONS);
    }
